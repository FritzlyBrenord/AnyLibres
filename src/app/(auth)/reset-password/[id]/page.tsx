...
export default function ResetPasswordPage({
  ...
  useEffect(() => {
    const verifyToken = async () => {
      if (!tokenId || !token) {
        setError(t?.auth?.resetPassword?.invalidLinkTitle || "Lien de réinitialisation invalide ou manquant");
        setVerifying(false);
        return;
      }
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(data.error || t?.auth?.resetPassword?.invalidLinkTitle || "Token invalide ou expiré");
        }
        ...
        setError(
          err instanceof Error
            ? err.message
            : (t?.auth?.resetPassword?.invalidLinkTitle || "Lien de réinitialisation invalide ou expiré"),
        );
      } finally {
        setVerifying(false);
      }
      ...
      <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-slate-900 via-indigo-900 to-purple-900 overflow-hidden relative">
        ...
        <div className="text-center relative z-10">
          ...
          <p className="text-slate-300">
              {t?.auth?.resetPassword?.verifying || "Vérification du lien..."}
          </p>
        </div>
      </div>
    );
  ...
  if (!tokenValid && !verifying) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-slate-900 via-indigo-900 to-purple-900 overflow-hidden relative">
        <Background />
        <div className="z-10 max-w-md w-full bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl p-8 shadow-2xl relative">
          <div className="text-center">
            <div className="mx-auto w-16 h-16 bg-red-500/20 rounded-full flex items-center justify-center mb-6 ring-1 ring-red-500/40">
              <AlertCircleIcon />
            </div>
            <h2 className="text-2xl font-bold text-white">
              {t?.auth?.resetPassword?.invalidLinkTitle || "Lien invalide ou expiré"}
            </h2>
            <p className="mt-2 text-slate-300">
              {t?.auth?.resetPassword?.invalidLinkDesc || "Ce lien de réinitialisation est invalide ou a expiré. Veuillez demander un nouveau lien."}
            </p>
          </div>
          ...
          <div className="mt-8 text-center">
            ...
            >
              <ArrowLeftIcon />
              <span className="ml-2">{t?.auth?.resetPassword?.requestNewLink || "Demander un nouveau lien"}</span>
            </Link>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-slate-900 via-indigo-900 to-purple-900 overflow-hidden relative">
      ...
      <div className="z-10 max-w-md w-full">
        ...
        <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl p-8 shadow-2xl">
            ...
            {email && <p className="text-slate-300">{t?.auth?.resetPassword?.forEmail?.replace("{email}", email) || `Pour ${email}`}</p>}
            </div>
