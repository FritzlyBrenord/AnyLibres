...
function Settings({ isDark = false }: SettingsProps) {
  ...
  const handleSaveSettings = async () => {
    ...
    if (
      !window.confirm(
        "Êtes-vous sûr de vouloir enregistrer ces modifications ? Cela affectera l'ensemble de la plateforme.",
      )
    ) {
      return;
    }
  ...
  const handleAddCategoryFee = () => {
    ...
    if (
      !window.confirm(
        `Ajouter des frais de ${categoryFee}% pour la catégorie "${category?.name.fr || selectedCategory}" ?`,
      )
    ) {
      return;
    }
  ...
  const handleRemoveCategoryFee = (categoryId: string) => {
    ...
    if (
      !window.confirm(
        `Supprimer les frais spécifiques pour la catégorie "${category?.name.fr || categoryId}" ?`,
      )
    ) {
      return;
    }
  ...
  const handleAddCountryFee = () => {
    ...
    if (
      !window.confirm(
        `Ajouter des frais de ${countryFee}% pour le pays "${country?.name || selectedCountry}" ?`,
      )
    ) {
      return;
    }
  ...
  const handleRemoveCountryFee = (countryCode: string) => {
    ...
    if (
      !window.confirm(
        `Supprimer les frais spécifiques pour le pays "${country?.name || countryCode}" ?`,
      )
    ) {
      return;
    }
  ...
  const handleAddLocationTypeFee = () => {
    ...
    if (
      !window.confirm(
        `Ajouter des frais de ${locationTypeFee}% pour le type "${locType?.name || selectedLocationType}" ?`,
      )
    ) {
      return;
    }
  ...
  const handleRemoveLocationTypeFee = (locationType: string) => {
    ...
    if (
      !window.confirm(
        `Supprimer les frais spécifiques pour le type "${locType?.name || locationType}" ?`,
      )
    ) {
      return;
    }
    ...
    return (
      <div className="flex flex-col items-center justify-center h-96 text-center">
        ...
        <p className="max-w-md text-gray-600 dark:text-gray-400">
          Vous n'avez pas la permission de voir les paramètres de la plateforme.
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="animate-fade-in mx-20">
        <h1
      ...
      {/* Tabs */}
      <div
        className={`border-b ${isDark ? "border-white/10" : "border-slate-200"} sticky top-0 bg-inherit z-10 mx-20`}
      >
      ...
      {/* Content */}
      <div
        className={`rounded-2xl shadow-xl overflow-hidden ${isDark ? "bg-slate-900/50 border border-white/10" : "bg-white border border-slate-200"}`}
      >
        <div className="p-6 md:p-8">
          ...
          {activeTab === "global" && (
            <div className="space-y-6">
              <div>
                ...
                <div
                  className={`mb-8 p-5 rounded-2xl border ${isDark ? "bg-indigo-500/5 border-indigo-500/20" : "bg-indigo-50 border-indigo-100"}`}
                >
                  <p
                    className={`text-sm font-semibold flex items-center gap-2 ${isDark ? "text-indigo-400" : "text-indigo-700"}`}
                  >
                    <span className="text-lg">💡</span>
                    <span>
                      Ces frais de commission sont payés par :{" "}
                      <span
                        className={`uppercase tracking-wider px-2 py-0.5 rounded-md ${isDark ? "bg-indigo-500/20" : "bg-white"}`}
                      >
                        {settings.global_fee_paid_by === "client" &&
                          "Le Client"}
                        {settings.global_fee_paid_by === "provider" &&
                          "Le Prestataire"}
                        {settings.global_fee_paid_by === "split" &&
                          "Les Deux (50/50)"}
                      </span>
                    </span>
                  </p>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="fixed">
                        Montant fixe ({defaultCurrency?.symbol || "€"})
                      </option>
                    </select>
                  </div>
                  ...
                  <div>
                    ...
                    <p
                      className={`text-xs mt-2 ${isDark ? "text-slate-500" : "text-slate-400"}`}
                    >
                      Montant minimum de commission par transaction. Si la
                      commission calculée est inférieure, c&apos;est ce montant
                      qui sera appliqué.
                    </p>
                    <div
                      className={`mt-3 p-3 rounded-lg text-xs font-bold inline-block ${isDark ? "bg-indigo-500/10 text-indigo-400" : "bg-indigo-50 text-indigo-600"}`}
                    >
                      PAYÉ PAR :{" "}
                      {settings.global_fee_paid_by === "client"
                        ? "Le Client"
              ...
              <div
                className={`border-t pt-8 mt-8 ${isDark ? "border-white/10" : "border-slate-100"}`}
              >
                <h2
                  className={`text-xl font-black mb-6 ${isDark ? "text-white" : "text-slate-900"}`}
                >
                ...
                <div
                  className={`mb-8 p-5 rounded-2xl border ${isDark ? "bg-orange-500/5 border-orange-500/20" : "bg-orange-50 border-orange-100"}`}
                >
                  <p
                    className={`text-sm font-semibold flex items-center gap-2 ${isDark ? "text-orange-400" : "text-orange-700"}`}
                  >
                    <span className="text-lg">💰</span>
                    <span>
                      Ces frais sont TOUJOURS payés par :{" "}
                  ...
                  <p
                    className={`text-xs mt-2 ml-7 ${isDark ? "text-slate-500" : "text-orange-600/70"}`}
                  >
                    Appliqués quand le prestataire retire son argent vers son
                    compte bancaire.
                  </p>
                </div>
                ...
                <div
                  className={`mt-6 p-4 rounded-xl border ${isDark ? "bg-slate-800/50 border-white/5" : "bg-slate-50 border-slate-100"}`}
                >
                  <p
                    className={`text-xs leading-relaxed ${isDark ? "text-slate-400" : "text-slate-600"}`}
                  >
                    <span className="font-bold mr-1">💡 Note :</span>
                    Une fois la limite atteinte, le prestataire devra attendre
                    que 24 heures se soient écoulées depuis son premier retrait
                    de la série avant de pouvoir en effectuer un nouveau.
                  </p>
                </div>
              </div>
            </div>
          ...
          {activeTab === "category" && (
            <div className="space-y-6">
              <div>
                ...
                {/* Formulaire d'ajout */}
                <div
                  className={`p-6 rounded-2xl mb-8 border ${isDark ? "bg-slate-800/40 border-white/5" : "bg-slate-50 border-slate-200"}`}
                >
                          ...
                          >
                            <div>
                              ...
                              <div className="flex items-center gap-2 mt-1">
                                <span
                                  className={`text-xs font-bold px-2 py-0.5 rounded-full ${isDark ? "bg-indigo-500/20 text-indigo-400" : "bg-indigo-50 text-indigo-600"}`}
                                >
                                  commission: {fee}%
                                </span>
                              </div>
                            ...
                            <button
                              onClick={() =>
                                handleRemoveCategoryFee(categoryId)
                              }
                    ...
                    <div className="md:col-span-2">
                      ...
                      >
                        ...
                        {countries
                          .filter(
                            (country) =>
                              !settings.fee_by_location[country.code],
                          )
                          .map((country) => (
                      ...
                      ([countryCode, fee]) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <button
                              onClick={() =>
                                handleRemoveCountryFee(countryCode)
                              }
                              className={`p-2 rounded-xl transition-all ${
                                isDark
                                  ? "text-red-400 hover:bg-red-500/10"
            ...
            <div className="space-y-6">
              <div>
                ...
                {Object.keys(settings.fee_by_location_type).length > 0 ? (
                  ...
                  <div
                    className={`text-center py-12 ${isDark ? "text-slate-500" : "text-gray-500"}`}
                  >
          ...
          <div
            className={`mt-8 pt-6 border-t ${isDark ? "border-white/10" : "border-slate-100"}`}
          >
            <button
      ...
      <div
        className={`rounded-2xl p-6 border ${isDark ? "bg-indigo-500/5 border-indigo-500/10" : "bg-blue-50 border-blue-100"}`}
      >
        <h3
          className={`text-lg font-bold mb-3 ${isDark ? "text-indigo-300" : "text-blue-900"}`}
        >
          Ordre de priorité des frais
        </h3>
        <ol
          className={`list-decimal list-inside space-y-2 text-sm ${isDark ? "text-slate-400" : "text-blue-800"}`}
        >
          <li>
        ...
        <p
          className={`text-xs mt-4 italic ${isDark ? "text-slate-500" : "text-blue-600"}`}
        >
