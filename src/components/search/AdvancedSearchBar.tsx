...
export function AdvancedSearchBar() {
      ...
      try {
        ...
        if (query.length === 1) {
          ...
          if (/^[A-Z]$/.test(char)) {
            newSuggestions.push({
              ...
              text: `${t?.search?.dynamic?.startsWith || "Services commençant par"} "${char}"`,
              type: "letter",
              score: 1000,
            ...
            newSuggestions.push({
              ...
              text: `${t?.search?.dynamic?.contains || "Services contenant"} "${char}"`,
              type: "letter",
              score: 900,
            ...
            newSuggestions.push({
              id: `number-${char}`,
              text: `${t?.search?.dynamic?.priceContains || "Services avec prix contenant"} "${char}"`,
              type: "number",
              score: 950,
        ...
        if (priceMatch) {
          ...
          newSuggestions.push({
            ...
            text: `${t?.search?.dynamic?.priceAround || "Services autour de"} ${price}`,
            type: "price",
            score: 800,
          ...
          newSuggestions.push({
            id: `price-max-${price}`,
            text: `${t?.search?.dynamic?.priceLess || "Services moins de"} ${price}`,
            type: "price",
            score: 750,
            icon: <DollarSign className="w-4 h-4" />,
          });
        ...
        if (query.length >= 2) {
          const response = await fetch(
            `/api/services/suggest?q=${encodeURIComponent(query)}&limit=10`,
          );
          ...
          if (response.ok) {
            ...
            if (data.providers && data.providers.length > 0) {
              data.providers.forEach(
                (provider: ProviderProfile, index: number) => {
                  newSuggestions.push({
                    id: `provider-${provider.id}`,
                    text: `${t?.search?.dynamic?.servicesOf || "Services de"} ${provider.company_name || provider.profile?.display_name || "Provider"}`,
                    type: "provider",
                    score: 600 - index,
                    provider,
        ...
        if (query.length >= 2) {
          ...
          newSuggestions.push({
            ...
            text: `${t?.search?.dynamic?.allStarting || "Tous les services commençant par"} "${firstChar}"`,
            type: "letter",
            score: 400,
    ...
    if (suggestion.type === "service" && suggestion.service) {
      ...
      if (priceMatch) {
        ...
        if (suggestion.id.includes("max")) {
          ...
          router.push(
            `/search?q=&minPrice=${price * 0.8}&maxPrice=${price * 1.2}`,
          );
        }
      }
    } else {
  ...
  const activeFiltersCount =
    Object.values(filters.searchIn).filter(Boolean).length +
          ...
          <div className="flex items-center gap-2 p-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder={
                  ...
                  "Recherchez par titre, description, prix, provider, lettre..."
                }
                className="w-full pl-12 pr-12 py-4 rounded-xl border-0 focus:outline-none focus:ring-2 focus:ring-amber-400/50 text-lg bg-transparent text-gray-900"
              />
              {searchQuery && (
                <button
                  type="button"
            ...
            >
              ...
              <span className="hidden sm:inline">
                {t?.home?.hero?.searchButton || "Rechercher"}
              </span>
            </Button>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                  <h3 className="text-sm font-semibold text-slate-700 mb-2 flex items-center gap-2">
                    ...
                    {t?.search?.searchIn || "Rechercher dans"}
                  </h3>
                    ...
                    {Object.entries(filters.searchIn).map(([key, value]) => (
                      ...
                      >
                        ...
                        <span className="text-sm text-slate-700 capitalize">
                          {t?.search?.filters?.[
                            key as keyof typeof t.search.filters
                          ] || key}
                        </span>
                        <input
                          type="checkbox"
                          checked={value}
                          onChange={() =>
                            toggleFilter(key as keyof FilterOptions["searchIn"])
                          }
                          className="sr-only"
                        />
                      </label>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <input
                      ...
                      className="w-full px-3 py-2 rounded-lg border text-gray-900 border-slate-300 focus:outline-none focus:ring-2 focus:ring-amber-400 text-sm"
                    />
                    <input
                      type="number"
                      ...
                      className="w-full text-gray-900 px-3 py-2 rounded-lg border border-slate-300 focus:outline-none focus:ring-2 focus:ring-amber-400 text-sm"
                    />
                  </div>
                </div>
      ...
      {showSuggestions && suggestions.length > 0 && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden z-50 animate-slide-down max-h-[500px] overflow-y-auto">
          ...
          <div className="p-2">
            ...
            {searchQuery.length === 0 && recentSearches.length > 0 && (
              <div className="mb-2">
                <div className="px-4 py-2 text-xs font-semibold text-slate-500 uppercase flex items-center gap-2">
                  ...
                  {t?.search?.recentSearches || "Recherches récentes"}
                </div>
                {suggestions
                  .filter(
                    (s) =>
                      s.type === "exact" && recentSearches.includes(s.text),
                  )
                  .map((suggestion) => (
                    <SuggestionItem
                      key={suggestion.id}
            ...
            {suggestions.filter(
              (s) => s.type === "letter" || s.type === "number",
            ).length > 0 && (
              <div className="mb-2">
                <div className="px-4 py-2 text-xs font-semibold text-slate-500 uppercase flex items-center gap-2">
                  <Sparkles className="w-3 h-3" />
                  {t?.search?.smartFilters || "Filtres intelligents"}
                </div>
                {suggestions
                  .filter((s) => s.type === "letter" || s.type === "number")
                  .map((suggestion) => (
                    <SuggestionItem
                      key={suggestion.id}
                      suggestion={suggestion}
            ...
            {suggestions.filter((s) => s.type === "exact").length > 0 &&
              searchQuery.length > 0 && (
                <div className="mb-2">
                  <div className="px-4 py-2 text-xs font-semibold text-slate-500 uppercase flex items-center gap-2">
                    <Zap className="w-3 h-3 text-yellow-500" />
                    {t?.search?.exactMatches || "Correspondances exactes"}
                  </div>
                  {suggestions
            ...
            {suggestions.filter(
              ...
              <div>
                ...
                {suggestions
                  .filter((s) => s.type === "similar" || s.type === "price")
                  .map((suggestion) => (
                    <SuggestionItem
                      key={suggestion.id}
                      suggestion={suggestion}
    ...
    >
      ...
      {showPrice && (
        <span className="text-sm font-semibold text-slate-600 group-hover:text-amber-600 flex-shrink-0">
          {showPrice.base_price_cents
            ? `${(showPrice.base_price_cents / 100).toFixed(0)}`
            : ""}
        </span>
      )}
    </button>
